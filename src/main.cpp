...
uint256 hashGenesisBlock("0x08c2eb77280ae769941a90da15c0689582c0c98832f8a6b11e816680a1b1b91a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Diamint: starting difficulty is 1 / 2^12
...
{
    // Diamint: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1260000 blocks, which will occur approximately every 2.4 years
    nSubsidy >>= (nHeight / 1260000); // Diamint: 1260k blocks in ~2.4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.25 * 24 * 60 * 60; // Diamint: 0.25 days
static const int64 nTargetSpacing = 1.44 * 60; // Diamint: 1.44 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Diamint: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd7e926526c0974dab9479d67e7c6f0071fc373bf1e1e132c9a1326a2a9896f88");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Muenchen, 27.02.2018, Niemand wollte Irans Airbus betanken";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048243b4417ae316c827e24cbc7539b4f4a5d30d1917feb8fcb4f45448b2ed1519fb354e537f5306de19a5b10ab93135a9977b8b5275f13767daf3223f3c3c5abc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520185571;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085301254;

        if (fTestNet)
        {
            block.nTime    = 1520185484;
            block.nNonce   = 385740108;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9457e39d5a66fe81e94fd8f6db71c92f27ecb4385dc66a95381ce65520f8c858"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb7, 0xf7, 0xc1, 0xd2 }; // Diamint: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DiamintMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DiamintMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DiamintMiner(CWallet *pwallet)
{
    printf("DiamintMiner started\n");
    ...
    try { loop {
        ...
        printf("Running DiamintMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DiamintMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DiamintMiner, pwallet));
}
